...
export function DrillCard({ drill, defaultExpanded = false }: { drill: DrillPlan; defaultExpanded?: boolean }) {
  ...
  return (
    <article className="workspace-panel p-4 md:p-6">
      <header>
        <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
          <h2 className="display-i text-[22px] leading-none text-cream">{drill.title} · {drill.duration}</h2>
          <span className="rounded-[3px] border border-wire bg-surface-2 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-text-faint">{drill.type}</span>
        </div>
