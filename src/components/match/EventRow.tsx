...
export function EventRow({ time, icon, iconTint = "default", team, title, subtitle, state, focused, onPlay, onConfirm, onHide }: {
  time: string;
  icon: StatIconName;
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "44px 32px minmax(0,1fr) auto", gap: "10px", alignItems: "center", padding: "10px 12px", border: "1px solid var(--wire)", borderRadius: "10px", background: state === "confirmed" || focused ? "var(--surface-2)" : "none", borderLeft: state === "confirmed" ? "3px solid var(--reaction-good)" : "1px solid var(--wire)", marginBottom: "8px", opacity: state === "hidden" ? 0.4 : 1 }}>
      ...
      <div style={{ minWidth: 0 }}><div style={{ fontSize: "12.5px", fontWeight: 600, color: "var(--text)", display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}><span style={{ width: "8px", height: "8px", borderRadius: "50%", background: team === "A" ? "var(--team-a)" : "var(--team-b)" }} />{title}</div>{subtitle && <div style={{ fontSize: "11px", color: "var(--text-faint)", marginTop: "3px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{subtitle}</div>}</div>
